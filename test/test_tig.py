...
from speach.tig import IGStreamReader, TTLTokensParser
...
class TestTokenizer(unittest.TestCase):
    ...
    def test_tokenizer(self):
        ...
        tokens = tig.tokenize(token_string)
        glosses = tig.tokenize(gloss_string)
        sent.tokens = tokens
    ...
    def test_tokenizing_special_chars(self):
        ...
        tokens = tig.tokenize('some\\ thing is a word .')
...
class TestFurigana(unittest.TestCase):
    ...
    def test_parse_furigana(self):
        ...
        self.assertEqual(rubytext.to_html(), '{<ruby><rb>漢字</rb><rt>かんじ</rt></ruby>}')
        # %
        rubytext = tig.parse_furigana('{％/パーセント}')
        self.assertEqual(str(rubytext), '％')
    ...
    def test_TTL_tokenizer(self):
        ...
        self.assertEqual(expected, actual)
        # test parse_ruby
        actual = tig.make_ruby_html('ケーキ を {食/た}べた 。')
...
''')
    ...
    def test_read_header(self):
        ...
        with chio.open(inpath) as infile:
            meta = tig.IGStreamReader._read_header(infile)
    ...
    def test_read_file(self):
        ...
        self.assertEqual(tig.make_ruby_html(s1.tokens), s1_furi)
        self.assertEqual(tig.make_ruby_html(s2.tokens), s2_furi)

    def test_ttlig_auto(self):
        inpath = JP_IMPLICIT
        sents = tig.read(inpath)
        s = sents[0]
    ...
    def test_read_empty_file(self):
        ...
        sents = tig.read_stream(instream)
        self.assertEqual(sents, [])

    def test_read_invalid_ttlig(self):
        instream = io.StringIO('')
        self.assertRaises(Exception, lambda: tig.read_stream(instream))
